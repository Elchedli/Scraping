...
(async function main(){
    try{
        ...
        var data = fs.readFileSync('third.txt', 'utf8');
        if(data == ""){
            data = 0;
        }
        mangabinary = await fs.promises.readFile('third.json');
        ...
        for (var i = data, len = mangalist.length; i < len; i++) {
            ...
            fs.writeFileSync('mangaliste5/'+kharbel+'.json',manga);
            fs.writeFileSync('third.txt',i+1);
        }
    }catch(e){
