...
(async function main(){
    try{
        ...
        var data = fs.readFileSync('fourth.txt', 'utf8');
        if(data == ""){
            data = 0;
        }
        mangabinary = await fs.promises.readFile('fourth.json');
        ...
        for (var i = data, len = mangalist.length; i < len; i++) {
            ...
            fs.writeFileSync('mangaliste6/'+kharbel+'.json',manga);
            fs.writeFileSync('fourth.txt',i+1);
        }
    }catch(e){
