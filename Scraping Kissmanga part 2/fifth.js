...
(async function main(){
    try{
        ...
        var data = fs.readFileSync('fifth.txt', 'utf8');
        if(data == ""){
            data = 0;
        }
        mangabinary = await fs.promises.readFile('fifth.json');
        ...
        for (var i = data, len = mangalist.length; i < len; i++) {
            ...
            fs.writeFileSync('mangaliste7/'+kharbel+'.json',manga);
            fs.writeFileSync('fifth.txt',i+1);
        }
    }catch(e){
