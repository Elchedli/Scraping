...
(async function main(){
    try{
        ...
        var data = fs.readFileSync('second.txt', 'utf8');
        if(data == ""){
            data = 0;
        }
        mangabinary = await fs.promises.readFile('second.json');
        ...
        for (var i = data, len = mangalist.length; i < len; i++) {
            ...
            fs.writeFileSync('mangaliste4/'+kharbel+'.json',manga);
            fs.writeFileSync('second.txt',i+1);
        }
    }catch(e){
